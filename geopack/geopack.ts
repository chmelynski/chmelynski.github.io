...
export function GeopackOsm(osm: Osm, bounds: Box): ArrayBuffer {
	
	var quantization = 256 * 256;
	
	var nLines = 0;
	var nPoints = 0;
	for (var key in osm.ways) { nLines++; nPoints += osm.ways[key].nodes.length; }
	...
	view.setFloat64(k, osm.bounds.minLat); k += 8;
	view.setFloat64(k, osm.bounds.maxLat); k += 8;
	view.setFloat64(k, osm.bounds.minLng); k += 8;
	view.setFloat64(k, osm.bounds.maxLng); k += 8;
	
	view.setFloat64(k, xScale); k += 8;
	view.setFloat64(k, yScale); k += 8;
	view.setFloat64(k, xTrans); k += 8;
	view.setFloat64(k, yTrans); k += 8;
	
	view.setUint32(k, nLines); k += 4;
	
	for (var key in osm.ways)
	{
		var way = osm.ways[key];
		
		if (way.nodes.length >= 256) { continue; }
		
		view.setUint8(k, way.nodes.length); k += 1;
		...
		for (var j = 0; j < way.nodes.length; j++)
		{
			...
			view.setUint16(k, x); k += 2;
			view.setUint16(k, y); k += 2;
		}
	}
	
	return buffer;
}
function UnpackToOsm(buffer: ArrayBuffer): Osm { return null; }
export function DrawGeopack(ctx: CanvasRenderingContext2D, mercator: Mercator, buffer: ArrayBuffer): void {
	
	var metersPerLat = 111321;
	var metersPerLng = 111321 * Math.cos(mercator.lat / 180 * Math.PI);
	
	function ProjX(tx: number): number {
		var lng = tx * xScale + xTrans;
		var x = (lng - mercator.lng) * metersPerLng / mercator.metersPerPixel;
		return x;
	}
	function ProjY(ty: number): number {
	...
	var xScale = view.getFloat64(k); k += 8;
	var yScale = view.getFloat64(k); k += 8;
	var xTrans = view.getFloat64(k); k += 8;
	var yTrans = view.getFloat64(k); k += 8;
	
	var nLines = view.getUint32(k); k += 4;
	
	for (var i = 0; i < nLines; i++)
	{
		var nPoints = view.getUint8(k); k += 1;
		
		for (var j = 0; j < nPoints; j++)
		{
			var x = ProjX(view.getUint16(k)); k += 2;
			var y = ProjY(view.getUint16(k)); k += 2;

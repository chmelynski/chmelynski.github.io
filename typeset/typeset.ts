...
// Typeset.en = { leftmin : 2 , rightmin : 2 , patterns : { 3 : <string> , 4 : <string> , ... } }
// Typeset.en is fed to the constructor Hypher(language)
...
// attempting to assign to Typeset.en made typescript complain, so we're just going to inline the language
var en = {
	leftmin : 2,
	rightmin : 2,
	patterns : {
...
interface HypherTrieNode {
	_points: number[];
	[index: number]: HypherTrieNode;
}
...
class Hypher {
	...
	createTrie(patternObject: LanguagePatterns): HypherTrieNode {
		
		var tree = { _points: [] };
		
		for (var size in patternObject)
		{
			if (patternObject.hasOwnProperty(size))
			{
				var patterns = patternObject[size].match(new RegExp('.{1,' + (+size) + '}', 'g'));
	...
	hyphenate(word: string): string[] {
		...
		var characters;
		var characterPoints = [];
		var originalCharacters;
		var node;
		var points = [];
		var wordLength;
		var nodePoints;
		var nodePointsLength;
		var m = Math.max;
		var trie = this.trie;
		...
		for (var i = 0; i < wordLength; i += 1) { points[i] = 0; characterPoints[i] = characters[i].charCodeAt(0); }
		
		for (var i = 0; i < wordLength; i += 1)
		{
			node = trie;
			
			for (var j = i; j < wordLength; j += 1)
			{
				...
				if (node)
				{
					...
					if (nodePoints)
					{
						for (var k = 0, nodePointsLength = nodePoints.length; k < nodePointsLength; k += 1)
						{
		...
		for (var i = 1; i < wordLength - 1; i += 1)
		{
...
type TextMeasurer = (str: string) => number;
...
function Justify(text: string, measureText: TextMeasurer, options: Options): TypesetNode[] {
	...
	for (var i = 0; i < words.length; i++)
	{
		...
		if (hyphenated.length > 1 && word.length > 4)
		{
			for (var k = 0; k < hyphenated.length; k++)
			{
				var part = hyphenated[k];
				nodes.push(box(measureText(part), part));
				if (k !== hyphenated.length - 1) { nodes.push(penalty(hyphenWidth, hyphenPenalty, 1)); }
			}
		}
		else
		{
...
export function Typeset(text: string, lineWidths: number[], measureText: TextMeasurer, options: Options = {}): Placement[] {
	var nodes = Justify(text, measureText, options);
